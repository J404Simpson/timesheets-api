...
exports.syncHolidayEntriesForEmployee = syncHolidayEntriesForEmployee;
exports.syncHolidayEntriesForAllEmployeesForYear = syncHolidayEntriesForAllEmployeesForYear;
exports.startHolidayYearEndScheduler = startHolidayYearEndScheduler;
...
function hasLeaveSignature(entry) {
    if (entry.project_id === LEAVE_PROJECT_ID)
        return true;
    return (entry.notes ?? "").startsWith(LEAVE_NOTE_PREFIX);
}
function isTime(date, hour, minute = 0) {
    return date.getUTCHours() === hour && date.getUTCMinutes() === minute;
}
function shouldPrepareNextYear(now = new Date()) {
...
async function syncHolidayEntriesForEmployeeYear(prisma, employeeId, regionId, year, logger) {
    const summary = {
        year,
        employeeId,
        created: 0,
    ...
    for (const holiday of holidays) {
        ...
        const hasOnlyMatchingHoliday = existing.length === 1 &&
...
async function syncHolidayEntriesForEmployee(prisma, employeeId, logger) {
...
async function syncHolidayEntriesForAllEmployeesForYear(prisma, year, logger) {
    ...
    for (const employee of employees) {
        await syncHolidayEntriesForEmployeeYear(prisma, employee.id, employee.region_id ?? 1, year, logger);
    }
    logger?.info({ year, employeeCount: employees.length }, "Holiday entries synced for all employees");
}
function startHolidayYearEndScheduler(prisma, logger) {
...
//# sourceMappingURL=holidaySync.js.map