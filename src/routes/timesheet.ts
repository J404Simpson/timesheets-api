...
function toFiniteHourValue(value: unknown): number | null {
...
function parseWeeklyHoursPayload(value: unknown): WeeklyHoursPayload | null {
...
function calculateWeeklyHoursTotal(hours: WeeklyHoursPayload): number {
  return Number((
    hours.hours_monday +
    hours.hours_tuesday +
...
export default async function timesheetRoutes(fastify: FastifyInstance, opts: FastifyPluginOptions) {
  ...
  fastify.get("/entries/date-bounds", async (request, reply) => {
    ...
    try {
      ...
      const bounds = await prisma.entry.aggregate({
  ...
  fastify.patch(
    "/projects/:projectId/phases/:phaseId/activate",
    async (
      request: FastifyRequest<{ Params: { projectId: string; phaseId: string } }>,
      reply: FastifyReply
    ) => {
      ...
      try {
        ...
        if (!link) return reply.status(404).send({ error: "Project phase link not found" });
  ...
  fastify.get(
    ...
    async (
      ...
      try {
        const { includeInactive, forEntry, employeeId } = request.query as {
        ...
        let departmentIdFilter: number | null = null;
        if (forEntryFlag) {
          const user = (request as any).user;
          const object_id = user?.oid;
          if (!object_id) {
  ...
  fastify.patch("/entries/:id/leave-time", async (request, reply) => {
    ...
    const { startTime } = (request.body ?? {}) as { startTime?: string };
  ...
  fastify.put("/entries/:id", async (request, reply) => {
    ...
    try {
      ...
      const entryEmployee = await prisma.employee.findUnique({
